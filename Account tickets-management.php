...
// Decide which tickets to display:
// If the user is ADMINISTRATOR, TECHNICAL, or STAFF then show all tickets;
// otherwise, show only the tickets created by the logged in user.
$username = $_SESSION['username'];
$usertype = $_SESSION['usertype'];

if ($usertype === 'TECHNICAL') {
    // TECHNICAL sees only tickets assigned to them.
    $sql = "SELECT * FROM tbltickets WHERE assignedto = ? ORDER BY ticketnumber DESC";
    $stmt = mysqli_prepare($link, $sql);
    mysqli_stmt_bind_param($stmt, "s", $username);
}else {
    // Other users (e.g., USER) see only tickets they created.
...
function buildTable($result, $usertype) {
    if (mysqli_num_rows($result) > 0) {
      ...
      while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
          ...
          $timeCreated = date('H:i:s', strtotime($datetime));
          ...
          echo "<td>";
            ...
            if ($usertype === 'USER' || $usertype === 'STAFF'){
                        // Edit button
                        if (in_array($normalizedStatus, ['PENDING', 'ONGOING', "FOR APPROVAL"])){
